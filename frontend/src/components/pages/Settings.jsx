  ...
  const fetchAvailableTemplates = async () => {
    try {
      ...
      if (res.status === 200 && res.data.success) {
        setAvailableTemplates(res.data.templates || defaultTemplates);
      } else {
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-0.5 transition-colors group-hover:text-purple-600 dark:group-hover:text-purple-400">
                        {loadingTemplates ? (
                          ...
                          availableTemplates.find(t => t.template === selectedTemplate)?.displayName || 
                          selectedTemplate.charAt(0).toUpperCase() + selectedTemplate.slice(1) || 
                          'Select a template'
                        )}
                      </div>
                      {selectedTemplate && !loadingTemplates && availableTemplates.find(t => t.template === selectedTemplate)?.description && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 truncate transition-colors group-hover:text-gray-600 dark:group-hover:text-gray-300">
                          {availableTemplates.find(t => t.template === selectedTemplate)?.description}
                        </div>
                      )}
                    </div>
                    <motion.div
                      ...
                      <div className="max-h-80 overflow-y-auto custom-scrollbar">
                        {availableTemplates.map((template, index) => (
                          <motion.button
                            ...
                            whileHover={{ 
                              x: 4,
                              backgroundColor: selectedTemplate === template.template 
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                <motion.div 
                                  ...
                                  transition={{ duration: 0.2 }}
                                >
                                  {template.displayName || template.template.charAt(0).toUpperCase() + template.template.slice(1)}
                                </motion.div>
